...
def compute_rewards(
    ...
    # Component 1: Order correctness (0 or 0.4)
    # Reduced from 0.5 to make room for step rewards
    order_score = 0.4 if final_order == true_order else 0.0

    # Component 2: Rate constant accuracy (0–0.3) log scale
    if final_k and final_k > 0 and true_k > 0:
        log_error = abs(math.log10(final_k) - math.log10(true_k))
        # Full marks within 0.5 log units, zero at 2 log units
        k_score = 0.3 * max(0.0, 1.0 - (log_error / 2.0))
    else:
        k_score = 0.0
    ...
    # Component 4: Activation energy Task 3 only (0–0.15)
    ea_score = 0.0
    if task_id == 3 and final_activation_energy and final_activation_energy > 0:
        ea_error = abs(
        ...
        k_score = k_score * 0.75

    # Component 5: Accumulated step rewards (0–0.1 max)
    # Cap step rewards so they don't dominate final score
    step_bonus = min(0.1, accumulated_step_rewards)

    total = round(
        order_score + k_score + efficiency_score + ea_score + step_bonus, 4
    )
    total = min(1.0, total)
